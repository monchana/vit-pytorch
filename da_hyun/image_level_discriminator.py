...
from .grad_scaling import *
from fvcore.nn import sigmoid_focal_loss_jit

class Discriminator(nn.Module):
    def __init__(self, in_feature, alpha):
        ...
        self.grl = GradientScalarLayer(alpha)

    # Hyun Edit : originally alpha was 1.0, changed to -1
    # def forward(self, x, domain_target = False, alpha = -1):
    def forward(self, x, domain_target = False):
        x = self.grl(x)
        x = self.reducer(x) 
        x = torch.flatten(x, 1)
        if domain_target:
